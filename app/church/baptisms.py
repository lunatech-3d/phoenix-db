...
def edit_baptism(master, church_id, baptism_id=None, person_id=None, refresh=None):
    win = tk.Toplevel(master)
    BaptismEditor(win, church_id, baptism_id, person_id, refresh)
    win.grab_set()
...
class BaptismEditor:
    def __init__(self, master, church_id, baptism_id=None, person_id=None, refresh=None):
        self.master = master
        self.church_id = church_id
        self.baptism_id = baptism_id
    ...
    def setup_form(self):
        labels = ["Baptism Date", "Notes", "Curator Summary", "Tags"]
        self.entries = {}
    ...
    def load_data(self):
        self.cursor.execute(
            "SELECT person_id, baptism_date, officiant_id, notes, curator_summary, event_context_tags FROM Baptism WHERE baptism_id=?",
            (self.baptism_id,),
        )
        row = self.cursor.fetchone()
        if row:
            ...
            self.entries["Baptism Date"].insert(0, date or "")
    ...
    def save(self):
        if not self.person_id:
            ...
            return
        try:
            bdate, _ = parse_date_input(self.entries["Baptism Date"].get().strip()) if self.entries["Baptism Date"].get().strip() else (None, None)
        ...
        if self.baptism_id:
            self.cursor.execute(
                "UPDATE Baptism SET person_id=?, church_id=?, baptism_date=?, officiant_id=?, notes=?, curator_summary=?, event_context_tags=? WHERE baptism_id=?",
                data + (self.baptism_id,),
            )
        else:
            self.cursor.execute(
                "INSERT INTO Baptism (person_id, church_id, baptism_date, officiant_id, notes, curator_summary, event_context_tags) VALUES (?, ?, ?, ?, ?, ?, ?)",
                data,
            )
            self.baptism_id = self.cursor.lastrowid
        self.conn.commit()
        if self.refresh:
            self.refresh()
        self.master.destroy()