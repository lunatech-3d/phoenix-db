...
class EventEditor:
    ...
    def setup_form(self):
        labels = ["Event Type", "Event Date", "End Date", "Description", "Link URL", "Curator Summary", "Tags"]
        self.entries = {}
    ...
    def load_data(self):
        self.cursor.execute(
            """SELECT event_type, event_date, event_date_precision, end_date, end_date_precision,
                      description, person_id, link_url, curator_summary, event_context_tags
        ...
        if not row:
            return
        etype, sdate, sp, edate, ep, desc, pid, link, summ, tags = row
        ...
        if self.event_id:
            self.cursor.execute(
                """UPDATE Church_Event SET event_type=?, event_date=?, event_date_precision=?, end_date=?, end_date_precision=?,
                       description=?, person_id=?, link_url=?, curator_summary=?, event_context_tags=? WHERE church_event_id=?""",
                data[:-1] + (self.event_id,),
            )
        else:
            self.cursor.execute(
                """INSERT INTO Church_Event (event_type, event_date, event_date_precision, end_date, end_date_precision,
                       description, person_id, link_url, curator_summary, event_context_tags, church_id)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                data,
            )
